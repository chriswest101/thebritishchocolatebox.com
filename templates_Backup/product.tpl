{foreach from=$products key=key item=product}
    <article class="productContainer">
		<a href="{$product.prod_code}.php" class="image featured productImage" {*onclick="showModal('{$product.prod_code}');"*} style="cursor:pointer;"><img src="img/products/{$product.image_path}" alt="" width="100" /></a>
		<header>
			<h3><a href="{$product.prod_code}.php" {*onclick="showModal('{$product.prod_code}');"*} style="cursor:pointer;">{$product.name}</a></h3>
		</header>
